...
Dräc
v0.7, November 17, 2016: Reprint WIP: polishing English.
...
--READY-- 6. Second Implementation
--READY-- 7. Synthesis and notation
---WIP--- 8. PB Class
--------- 9. Conclusion
--------- A. Optimisations
==============================================================================
...
The `New : EndNew` block allows to instanciate a new object of the Class by declaring and initialising it.
...
The goal of this keyword is to create a new object and initialise it. These tasks are performed in the `New_ClassName` procedure, which is the main part of the `New` macro.

This procedure accepts a single argument, the one required by `Init_Mbers` for attributes initialisation.
...
The trick in the `New` macro is that it ends with the `Init_Mbers` keyword. This way, what the user has to add inside the `New : EndNew` block is simply the attributes initialisation. More on that in a moment though (<<Init_Mbers : EndInit_Mbers private block, Show me now!>>).
...
[IMPORTANT]
====
[[admonition-on-object-being-a-pointer]]With regard to what was studied until now, the object will always be a pointer. It doesn’t matter but it’s the consequence of the choice made in regrouping the access of the methods and the access of the members (<<EndClass keyword,What?! I don’t remember!>>).
